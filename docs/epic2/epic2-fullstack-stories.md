...
주문의 현재 상태만 보여주는 것이 아니라, 상태가 어떤 순서로 전이됐는지 이력 전체를 타임라인으로 보여준다. 이 화면이 완성되면 Saga의 비동기 흐름이 "PENDING → CONFIRMED" 또는 "PENDING → CANCELLED"라는 전이 경로로 UI에 시각화된다. Epic 1(Saga 백엔드)과 Epic 2(풀스택)가 가장 직접적으로 연결되는 지점이다.
...
- Story 5에서 생성한 주문 ID로 이력 조회 → `PENDING` 한 줄만 있는 상태 확인
