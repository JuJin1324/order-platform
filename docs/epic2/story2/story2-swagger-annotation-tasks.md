...
하나의 Task에 섞으면 리뷰 시 "의도 설명이 충분한가"와 "필드 설명이 충분한가"를 동시에 판단해야 해서 초점이 분산된다. Task 1을 먼저 머지하고 Swagger UI를 직접 확인한 뒤, "DTO 필드가 무슨 뜻인지 모르겠다"는 부족함을 Task 2에서 채우는 순서가 자연스럽다.
