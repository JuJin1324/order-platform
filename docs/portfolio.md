...
이것이 EDA에서 Result 타입(또는 Either)이 자연스럽게 요구되는 이유다. 실패를 예외가 아닌 반환 타입으로 표현하면, 성공과 실패 두 경로가 코드 구조에서 동등하게 보인다.
...
2. **멱등성 기반**: Kafka는 같은 메시지를 두 번 이상 전달할 수 있다. 동일한 주문에 대한 결제 요청이 재전달됐을 때, 이미 처리했다는 이력이 없으면 중복 처리가 발생할 수 있다.

이 문제도 Result 타입 도입으로 자연스럽게 해결된다. 실패도 반환 타입으로 표현하면, 실패 경로에서도 DB 저장을 강제하는 구조를 만들 수 있다.
