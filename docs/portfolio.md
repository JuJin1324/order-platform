...
결제 서비스(payment-service)에서 결제 실패 보상 흐름을 검증하다 예상치 못한 상황을 마주쳤다. 결제 금액이 한도를 초과하는 조건으로 주문을 생성했을 때, 결제가 실패해서 주문이 취소됐는데 결제 서비스 DB에 결제 레코드 자체가 존재하지 않았다.
...
결제 시도 자체는 발생했지만 DB에 흔적이 없으니, 결제가 실패한 건지 시도조차 없었던 건지 DB만 보고는 구분할 수 없다. "왜 주문이 취소됐는가"를 나중에 추적할 근거가 사라진 것이다.

해결 방향은 실패를 예외가 아닌 반환값으로 표현하는 것이다. 결제 처리 로직이 예외를 던지는 대신 "성공" 또는 "실패"를 담은 값을 반환하면, 실행 흐름이 중단되지 않는다. DB 저장 코드에 도달할 수 있고, 저장 후 반환값에 따라 성공 이벤트를 발행할지 실패 이벤트를 발행할지 결정하면 된다. Result 타입이 이 구조를 표현하는 수단이다.

```
현재: 결제 처리 → 예외 발생 → 흐름 중단 → DB 저장 안 됨
목표: 결제 처리 → Result(성공 or 실패) 반환 → DB 저장 → 이벤트 발행
```
