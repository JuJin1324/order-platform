...
Step 2b를 구현하면서 어색한 지점이 생겼다. 설계 수준에서는 재고 부족이라는 실패를 Kafka 이벤트로 발행하도록 했다. 그런데 실제로 재고 부족 여부를 판단하는 비즈니스 로직은 여전히 예외를 던지는 방식으로 구현되어 있었다. 그 결과 Kafka 이벤트를 발행하는 코드가 예외를 catch해서 이벤트로 번역하는 구조가 됐다.
