...
Step 2b를 구현하면서 어색한 지점이 생겼다. 설계 수준에서는 재고 부족이라는 실패를 Kafka 이벤트(`inventory-deduction-failed`)로 발행하도록 했다. 그런데 실제로 재고 부족 여부를 판단하는 비즈니스 로직은 여전히 예외를 던지는 방식으로 구현되어 있었다. 그 결과 Kafka 이벤트를 발행하는 코드가 예외를 catch해서 이벤트로 번역하는 구조가 됐다.
...
결제 금액이 한도를 초과하는 조건으로 보상 흐름을 검증하다 발견한 구조적 공백이다. 결제가 실패해서 주문이 취소됐는데, 결제 서비스 DB에 결제 레코드 자체가 존재하지 않았다.
