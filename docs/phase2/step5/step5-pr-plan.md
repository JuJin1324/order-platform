# Step 5 주문 생성 폼 + API 연결 PR 계획

이 문서는 "Step 4의 빈 뼈대에 주문 생성 기능을 실제로 어떻게 채울 것인가"를 정리한다. 이 Step이 머지되면 A 완료 기준(Swagger 완성 + React에서 실제 API 호출 가능)이 달성된다.

---

## 1. 왜 두 PR로 나누는가

Step 5는 Phase 2에서 난이도가 가장 높다. 폼 처리, API 호출, 상태 관리, 에러 처리가 동시에 들어온다. 한 번에 전부 구현하면 문제 발생 시 "어디서 막혔는가"를 분리하기 어렵다.

- **PR 1**: Happy path만 먼저 끝까지 연결 — "성공 흐름이 돈다"가 확정
- **PR 2**: 에러 처리 + UX 개선 — 이미 작동하는 흐름 위에 실패 분기를 얹음

이 순서로 가면 PR 1에서 폼 → API → 응답 표시의 기본 회로가 먼저 검증되고, PR 2는 순수하게 "실패 상황에서 UI가 반응하는가"에 집중할 수 있다.
...
- 주문 생성 후 주문 ID는 화면에 그대로 표시한다. 자동 navigation은 Step 7에서
...
- `hooks/useOrderCreate.ts` 구현
  ...
  - `error` 상태는 PR 2에서 채움 (구조만 선언)
- `pages/OrderCreatePage.tsx`에 폼 UI 추가
  - 필드: Swagger 문서에서 확인된 최소 필드 (예: `productId`, `quantity`)
  - 제출 버튼 + 로딩 표시
...
- 에러 메시지 UI (PR 2)
- 제출 버튼 중복 클릭 방지 고도화 (PR 2)
- 성공 후 상태 조회 페이지로 navigation (Step 7)
...
- 상태 조회 화면 — Step 6
- 주문 생성 후 상태 조회로 자동 이동 — Step 7
- React Hook Form 등 폼 라이브러리 — Phase 2 범위 밖
- `ErrorBoundary`, 에러 타입 정규화 — Phase 2 범위 밖
- 접근성(aria-live 등) — Phase 2 범위 밖

---

## 5. 권장 순서 요약

1. `useOrderCreate` Hook happy path 구현
2. 폼 UI 추가 + API 연결 확인
3. 주문 ID 화면 표시 + DB 레코드 확인
4. 에러 상태 구현
5. 에러 메시지 UI 추가
6. UX 디테일 정리 (중복 클릭 방지, 메시지 초기화)
