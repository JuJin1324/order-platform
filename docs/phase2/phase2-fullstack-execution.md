# Phase 2 — 풀스택 확장 실행 계획

> 작성일: 2026-04-16
> 기반 문서: `problems/order-platform/define-problem/next-steps-overview.md`
> 범위: Phase 2 (Swagger + React SPA) — B 완료 기준까지
...
- **1차 완료 기준 (A)**: Swagger 완성 + React에서 주문 생성 API 실제 호출 가능 상태 (Step 1~5)
...
Phase 2에서 Redux를 도입하지 않는다. 이유:

Phase 2의 상태는 세 가지가 전부다: 주문 생성 폼 입력값, API 응답으로 받은 주문 ID, 주문 상태 조회 결과. 페이지가 2개이고 두 페이지 사이에 주문 ID 하나만 넘기면 된다. `useState` + React Router의 `navigate(state)`로 충분히 해결된다.
...
Phase 2는 이 중 어느 것도 해당하지 않는다. 지금 Redux를 도입하면 React 자체를 배우는 데 써야 할 주의가 분산된다. 프로젝트가 커지면서 상태 공유 필요성이 생길 때 Context API 또는 Redux를 도입하는 것이 자연스러운 순서다.
...
| Step | 한 줄 요약 | A/B |
|---|---|---|
| Step 1 | Swagger UI를 백엔드에 붙인다 | A |
| Step 2 | Swagger 문서를 명세서 수준으로 완성한다 | A |
| Step 3 | React 앱을 만들고 백엔드와 연결한다 | A |
| Step 4 | 이후 구현의 뼈대를 잡는다 | A |
| Step 5 | 주문 생성 폼에서 실제 API를 호출한다 | A |
| Step 6 | 주문 상태를 조회하는 화면을 만든다 | B |
| Step 7 | 두 화면을 하나의 UX 흐름으로 잇는다 | B |
| Step 8 | Phase 2 결과물을 포트폴리오로 정리한다 | — |

---

## Step 1. Swagger 기본 연동
...
Swagger 기본 연동을 React보다 먼저 하는 이유는, React 개발 전에 API 명세서를 완성해두기 위해서다. Phase 1에서 백엔드가 이미 완성됐고 API 스펙이 확정된 상태이므로, React 개발 중 API 스펙이 바뀔 가능성이 없다. Swagger를 먼저 완성하면 React 구현 시 Swagger UI를 API 레퍼런스로 참조하며 작업할 수 있다.
...
- "코드를 보지 않고 Swagger UI만으로 이 API를 이해할 수 있는가?"를 현재 상태로 평가 → Step 2에서 개선할 기준점으로 메모

---

## Step 2. Swagger 어노테이션 정리
...
Step 1에서 기본 노출만 했던 Swagger 문서를 명세서 수준으로 완성한다. 각 엔드포인트의 의도, 요청/응답 형태, 에러 케이스를 어노테이션으로 정리한다. 이 문서가 React 개발 전체의 API 레퍼런스가 된다.

### 과정 맥락

Swagger 기본 연동(Step 1) 직후에 어노테이션 정리를 붙이는 이유는, Phase 1에서 백엔드 API 스펙이 이미 확정됐기 때문이다. React 개발 중 API 스펙이 바뀔 가능성이 없으므로 지금 완성해도 이중 작업이 없다. Swagger를 완성한 뒤 React로 넘어가면, 이후 모든 React 구현 단계에서 깔끔한 API 문서를 참조하며 작업할 수 있다.
...
- Step 1 TODO에서 메모한 "개선할 기준점"이 모두 반영됐는지 대조

---

## Step 3. SPA·React 기본 학습 + 프로젝트 생성 + 멀티모듈 통합
...
프로젝트를 생성하기 전에 SPA와 React의 기본 개념을 먼저 파악한다. 개념 없이 생성된 프로젝트 파일을 보면 무엇이 어디에 있는지, 왜 이 구조인지 이해하기 어렵다. 학습 후 프로젝트를 생성하면 Vite가 만들어준 파일 구조가 "왜 이렇게 되어 있는가"로 읽힌다. 이 Step이 끝나면 React 앱이 실행되고 백엔드와 통신할 준비가 된 상태가 완성된다.
...
학습을 Step 3에 두는 이유는, Step 4(아키텍처 세팅)에서 View·ViewModel·API 계층을 설계할 때 React 컴포넌트와 Custom Hook의 역할을 이해한 상태여야 구조 결정이 납득되기 때문이다. 학습 없이 Step 4로 넘어가면 "왜 Hook이 ViewModel인가"가 감각이 아닌 규칙으로만 남는다. 깊게 파고드는 학습이 아니라 Step 4 진입에 필요한 최소 맥락을 잡는 수준으로 범위를 제한한다.
...
View, ViewModel, API/데이터 계층이 명확히 분리된 구조를 잡는다. 이 분리가 없으면 컴포넌트 안에 API 호출, 상태 관리, 렌더링 로직이 뒤섞여 Step 5~7에서 코드를 읽고 수정하기 어려워진다. SPA 개념·React·프론트엔드 아키텍처 학습은 이 Step에서 구현하면서 함께 한다.
...
백엔드와 프론트엔드가 실제로 연결되는 첫 지점을 만든다. 이 Step이 완료되면 "Swagger에서 직접 API를 호출하는 것"과 "React UI의 폼을 통해 호출하는 것"의 차이를 체감할 수 있다. 여기까지가 A 완료 기준이다.

### 과정 맥락

주문 생성(POST)을 상태 조회(GET)보다 먼저 구현하는 이유는 데이터를 만들어야 조회할 수 있기 때문이다. 또한 POST 요청은 요청 바디 구성, Content-Type 설정, 에러 처리가 GET보다 복잡하다. 어려운 것을 먼저 해결하면 이후 단계가 수월해진다. API 클라이언트 모듈(Step 4)을 통해 호출하므로 컴포넌트 코드에 fetch 로직이 직접 들어가지 않는다.
...
주문의 현재 상태만 보여주는 것이 아니라, 상태가 어떤 순서로 전이됐는지 이력 전체를 타임라인으로 보여준다. 이 화면이 완성되면 Saga의 비동기 흐름이 "PENDING → CONFIRMED" 또는 "PENDING → CANCELLED"라는 전이 경로로 UI에 시각화된다. Phase 1(Saga 백엔드)과 Phase 2(풀스택)가 가장 직접적으로 연결되는 지점이다.

이벤트 소싱이 아니다. `OrderStatusHistory`는 상태 전이가 발생할 때마다 감사 로그(audit log)를 남기는 단순한 테이블이다. Order 엔티티의 현재 상태 컬럼은 그대로 유지한다.

### 과정 맥락

백엔드와 프론트엔드 변경을 같은 Step에 묶는 이유는, 이 두 변경이 "이력이 보이는 화면"이라는 하나의 완료 기준으로 묶이기 때문이다. 백엔드만 완성하고 프론트엔드가 없으면 이력 확인이 불가능하고, 프론트엔드만 먼저 만들면 연결할 API가 없다.
...
- Step 5에서 생성한 주문 ID로 이력 조회 → `PENDING` 한 줄만 있는 상태 확인
...
Step 5와 Step 6에서 만든 두 화면을 하나의 사용자 흐름으로 연결한다. "주문 생성 완료 → 상태 확인 페이지로 이동 → 새로고침으로 상태 확인"이 자연스럽게 이어지는 UX를 만든다. 이 Step이 끝나야 "풀스택 데모"로 보여줄 수 있는 형태가 된다.
...
graph LR
    ...
    WHY --> HOW["Phase 1과의 연결\nSaga 백엔드 위에\n풀스택을 쌓은 흐름"]
    HOW --> OUT["포트폴리오 문서\n(problems/order-platform/\nplan-execution/portfolio/)"]
```

### 의도 맥락

구현 결과물은 스스로 설명하지 않는다. Phase 2에서 무엇을 만들었는지, 왜 그 기술 스택을 골랐는지, Phase 1과 어떻게 이어지는지를 프리랜서 클라이언트의 시선으로 정리한다. 기술 구현과 포트폴리오 서술은 다른 작업이다. 구현이 끝난 직후에 작성해야 결정 맥락이 생생하게 남는다.
...
- [ ] Phase 2에서 만든 것(React SPA + Swagger)이 한 문단으로 설명됨
- [ ] 기술 스택 선택 근거가 포함됨 (Vite 선택 이유, Swagger 구조 설계 의도)
- [ ] Phase 1(Saga 백엔드)과의 연결이 서술됨 — "백엔드만 있던 프로젝트에 풀스택을 쌓은 흐름"
...
- Phase 1 포트폴리오(기존 작성본)와 연결이 자연스럽게 읽히는지 확인
...
| 날짜 | Step | 핵심 산출물 |
|---|---|---|
| Day 1 | Step 1 + Step 2 | Swagger UI 동작 + 어노테이션 정리 완성 |
| Day 2 | Step 3 + Step 4 | React 앱 실행 + 아키텍처 뼈대 완성 |
| Day 3 | Step 5 | 주문 생성 폼 + API 연결 **(A 완료)** |
| Day 4 | Step 6 | 상태 이력 테이블(백엔드) + 타임라인 화면(프론트엔드) |
| Day 5 | Step 7 | 전체 흐름 연결 **(B 완료)** |
| Day 6 | Step 8 | 포트폴리오 문서 완성 |

Step 5에 하루를 온전히 배정한 이유는, React를 처음 다루는 상황에서 폼 처리·상태 관리·에러 핸들링을 한 번에 구현하는 것이 이 Phase에서 가장 난이도가 높은 단계이기 때문이다. Day 4의 Step 6+7은 Step 5보다 단순한 흐름이므로 하루 안에 가능하다. Day 5는 구현이 끝난 직후 결정 맥락이 생생할 때 포트폴리오를 완성하는 날이다.

---

## 확인이 필요한 사항

1. **`GET /orders/{id}` 엔드포인트 존재 여부**: Step 6에서 상태 조회에 필요하다. order-service에 이미 구현되어 있는지 확인 필요. 없으면 Step 6 전에 백엔드에 추가해야 한다.
...
3. **주문 생성 API 요청 스펙**: `POST /orders` 요청 바디에 어떤 필드가 필요한지 (상품 ID, 수량, 사용자 ID 등) Step 5 폼 설계 전에 Swagger UI 또는 코드에서 확인한다.
