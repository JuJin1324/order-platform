...
Step 4는 아키텍처 뼈대를 잡는 작업이다. 의존성 방향이 명확하다.
...
- **PR 1**: 하위 계층(API + 타입)을 먼저 확정
- **PR 2**: 상위 계층(Hook + Page + Router)을 PR 1 위에 올림

반대 순서로 하면 "Hook이 `api/` 함수를 호출하는데 아직 타입이 없다" 같은 상황에서 타입을 임시로 채우게 되고, PR 1에서 다시 맞춰야 한다.
...
- React Router (PR 2)
- Page 컴포넌트 (PR 2)
- 에러 타입 정규화 — Step 5에서 실제 에러 처리와 함께
...
### PR 2. Hook / Page / Router 뼈대

#### 목표

Custom Hook과 페이지 컴포넌트의 빈 뼈대를 만들어, Step 5 구현이 이 위에 얹히는 형태가 되도록 한다.

#### 핵심 작업

- `frontend/src/hooks/useOrderCreate.ts`: 빈 Hook 뼈대
  - 반환 시그니처: `{ createOrder, isLoading, result, error }`
  - 내부 로직은 최소 (Step 5에서 구현)
- `frontend/src/hooks/useOrderStatus.ts`: 빈 Hook 뼈대 (Step 6에서 구현)
...
- 이 규칙이 Step 5~7에서 지켜져야 3계층 분리가 유지된다.

#### 이 PR에서 하지 않을 것

- 실제 폼 UI (Step 5)
- 페이지 간 navigation 로직 (Step 7)
- 에러 처리 UI (Step 5)
...
Step 5 구현이 새 폴더 생성 없이 이 뼈대에 로직만 채우는 형태로 가능해야 한다.

---

## 4. 이 계획에서 의도적으로 뒤로 미룬 것

- `ErrorBoundary` — Phase 2 범위 밖
- 전역 상태 관리 — 필요성이 명확해지기 전까지 미도입
- 공통 UI 컴포넌트 라이브러리 — Step 5에서 실제 UI가 생길 때 판단
- 프론트엔드 테스트(Vitest, React Testing Library) — Phase 2 범위 밖
