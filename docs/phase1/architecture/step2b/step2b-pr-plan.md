...
| **실패 레코드 영속화 (FAILED 상태 저장)** | 현재 결제 실패 시 `Payment` 레코드가 DB에 저장되지 않는다. `Payment.complete()`가 예외를 던지면 `paymentRepository.save()` 자체가 호출되지 않기 때문이다. EDA에서는 "결제 시도가 있었고 실패했다"는 사실 자체가 도메인 이벤트다. FAILED 상태로 저장되어야 (1) 왜 주문이 취소됐는지 DB만으로 추적 가능하고, (2) 같은 `ORDER_CREATED` 이벤트가 재전달될 때 중복 처리를 방어할 수 있다. Result 타입 도입 시 실패 경로에서도 자연스럽게 저장을 강제할 수 있다. |
| **`OrderStatus.FAILED` 및 `failOrder` 복원** | Step 2b에서 `OrderStatus.FAILED`와 `Order.fail()`, `OrderApplicationService.failOrder()`를 미리 추가했으나 실제로 호출하는 코드가 없어 삭제했다. `FAILED`는 "기술적 오류(메시지 유실, DLT 처리 등)"를 나타내는 상태로, Outbox 패턴·DLT 처리와 함께 Step 3에서 도입할 때 다시 추가한다. |
